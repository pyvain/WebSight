...
public final class IntersectionSet {
	 ...
	 */
	private HashMap<Integer, HashMap<Integer, Intersection>> intersections;

	/**
	 * Initializes an empty intersection set
	 */
	public IntersectionSet() {
		intersections = new HashMap<Integer, HashMap<Integer, Intersection>>();
	}
	...
	public void add(Intersection newInter) {
		Point p = newInter.getLocation();
		int x = p.getX();
		int y = p.getY();
		HashMap<Integer, Intersection> intersAtX = intersections.get(x);
		if (intersAtX == null) {
			intersAtX = new HashMap<Integer, Intersection>();
