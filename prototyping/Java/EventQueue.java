...
public final class EventQueue {
	 ...
	 */
	private HashMap<Point, Event> finder;
	...
	public EventQueue(Collection<Segment> segments) throws IllegalArgumentException {
		...
		events = new ArrayList<Event>(capacity);
		finder = new HashMap<Point, Event>(capacity);
		next = 0;
		// Adds each endpoint of each segment
		for (Segment s : segments) {
			add(new Event(s, Event.LEFT));
			add(new Event(s, Event.RIGHT));
		}
		// Sorts the events
	...
	private void add(Event event) throws IllegalArgumentException {
		...
		if (finder.putIfAbsent(location, event) != null) {
